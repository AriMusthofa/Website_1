...
.badge-metode.kas { background: #f0fdf4; color: #166534; }
...
.btn-bukti:hover { background: #1d4ed8; }
.no-bukti { color: #9ca3af; font-size: 13px; }
...
<tr>
    ...
    <td><?php
        $mt = $row['metode_bayar'] ?? '';
        if(!empty($mt)){
            $label = $metode_label[$mt] ?? strtoupper($mt);
            $cls   = ($mt === 'kas') ? 'badge-metode kas' : 'badge-metode';
            echo "<span class=\"$cls\">".htmlspecialchars($label)."</span>";
        } else {
            echo '<span class="no-bukti">-</span>';
        }
    ?></td>
    <td><?php
        $bukti = $row['bukti_pembayaran'] ?? '';
        if(!empty($bukti) && file_exists($bukti_dir.$bukti)){
            echo '<a href="'.$bukti_dir.htmlspecialchars($bukti).'" target="_blank" class="btn-bukti">Lihat Bukti</a>';
        } elseif(($row['metode_bayar'] ?? '') === 'kas'){
            echo '<span class="no-bukti">Bayar Tunai</span>';
        } else {
            echo '<span class="no-bukti">-</span>';
        }
    ?></td>
...
<tr>
    <td colspan="10" style="padding:35px;text-align:center;">Belum ada booking aktif.</td>
</tr>
<?php } ?>
...
<div id="area-cetak">
    ...
    <table class="dashboard-table" id="tabel-selesai">
        ...
        <tr class="row-selesai" data-tanggal="<?= htmlspecialchars($tgl_raw) ?>">
            ...
            <td><?= !empty($mt_r) ? htmlspecialchars($metode_label[$mt_r] ?? strtoupper($mt_r)) : '-' ?></td>
            <td class="no-print"><?php
                if(!empty($bukti_r) && file_exists($bukti_dir.$bukti_r)){
                    echo '<a href="'.$bukti_dir.htmlspecialchars($bukti_r).'" target="_blank" class="btn-bukti">Lihat Bukti</a>';
                } elseif($mt_r === 'kas'){
                    echo '<span class="no-bukti">Bayar Tunai</span>';
                } else {
                    echo '<span class="no-bukti">-</span>';
        ...
        <tr>
            <td colspan="9" style="padding:35px;text-align:center;">Belum ada booking selesai.</td>
        </tr>
        <?php } ?>
...
function filterSelesai(){
    ...
    allRows.forEach(function(row){
        ...
        if(show){
            var nominal = row.cells[8].textContent.replace(/[^0-9]/g, '');
